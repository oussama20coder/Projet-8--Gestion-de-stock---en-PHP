    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        for (var i = 0; i < studentsList.length; i++) {
          $(".studentsList").append('<li><h2>' + "ID: " + studentsList[i]["name"] + '</h2><h3>' + "First name  " + studentsList[i]["fathername"] + '</h3><h4>' + "Last name  : " + studentsList[i]["rollno"] + '</h4><h5>' + "Tel : " + studentsList[i]["degree"] + '</h5><h6>' + "Email: " + studentsList[i]["branch"] + '</h6><span class="update-btn">update</span><span class="delete-btn">delete</span><input type="hidden" value="" class="stdID"></li>');
        }
      }
    })
  }
